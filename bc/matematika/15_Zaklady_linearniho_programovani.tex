...
\begin{pitemize}
	...
	\item Vìty o dualitì (bez dùkazu)
\end{pitemize}
\end{e}

\ramcek{14cm}{Lineární programování je oznaèení pro úlohu maximalizovat jistou funkci $n$ reálnıch promìnnıch na mnoinì bodù polytopu v prostoru $\mathbb{R}^n$.}

Nejprve si udìlejme malı vılet do geometrie. {\it Polytop} je zobecnìním polygonu
(mnohoúhelníku) do vyšších dimenzích. Pro dimenzi $3$ se ale pouívá ještì speciální
...
Vzhledem k tomu, e tyto konvexní polytopy jsou prùnikem jistého mnoství poloprostorù,
mùeme je popsat maticovou rovnicí tvaru \[Ax\leq b,\] kde $A$ je matice øádu $m\times n$
a $m$ je poèet poloprostorù, jejich prùnikem je danı polytop,
a $n$ je dimenze podprostoru, ve kterém polytop máme.

{\it Simplex} je \uv{$n$--dimenzionální} trojúhelník (prùnik nìkolika poloprostorù). Podle rostoucí dimenze je to tedy po øadì bod, úseèka, trojúhelník, ètyøstìn, pentachoron (viz obrázek 1) atd. Mùe bıt omezenı i neomezenı.
...
Maticovì to mùeme zapsat ve tvaru $Ax\leq b$, kde $A$ je reálná matice
...
pokud existuje nìjakı bod splòující $x\geq 0$ a $Ax\leq b$. Jinak je zadání {\it nepøípustné}.
 \item Úloha je {\it neomezená}, pokud mùeme úèelovou funkcí dosáhnout na pøípustnıch bodech libovolnì veliké hodnoty.
Jinak je {\it omezená}.
\end{penumerate}
...
bodù obecnì mùe bıt více a øíkáme jim optimální øešení.
\end{e}
...
Omezující podmínku $Ax\leq b$ tedy zmìníme na $Ax=b$. Toho docílíme pøidáním jedné nezáporné promìnné pro kadou podmínku ( $\sum{x}<b$ je toti ekvivalentní $\sum{x}+y=b$ kde $y\in\mathbb{R}^+$). Tuto úpravu lze také zapsat jako $A:=(A\ I)$. Dále pøedpokládáme, e matice $A$ má lineárnì nezávislé øádky.
Pro podmnoinu indexù $I\subseteq \{1, 2, \dots, n\}$
...
\ramcek{14cm}{
{\it Báze} (a to nemá nic spoleèného s bází vektorového prostoru) je libovolná podmnoina indexù $B\subseteq \{1, 2, \dots, n\}$ taková, e matice $A_B$ je regulární.
...
$A_Bx_B=b$ a $x_i=0$ pro kadé $i\in\{1,2,\dots,m\}\backslash B$.
}
...
\end{e}
%\begin{e}{Dùkaz}{0}{0}
%Jako $K$ oznaème mnoinu tìch indexù, pro které je $x_j>0$. Implikace $\Rightarrow$ je jednoduchá,
...
%Nyní ukame implikaci $\Leftarrow$.
%\end{e}

\begin{e}{Vìta}{0}{0}
Nech $B$ je $m$--prvková indexová mnoina $B\subseteq \{1,\dots, n\}$ a $A_B$ je regulární. Pak existuje
...
øešení a je-li zároveò úèelová funkce na mnoinì pøípustnıch øešení omezená, pak existuje
...
\end{e}}
Tato vìta má obrovskou dùleitost, nebo je jasné, e {\bf bazickıch øešení je jen koneènì mnoho}.
Ukame si fungování simplexové metody na konkrétním pøíkladu.
\begin{e}{Pøíklad}{0}{0}
...
Ponechejme $x_1=0$ a zvìtšeme $x_2$ z $0$ na $1$ (jednièka je nejlepší moná, viz první rovnici a $x_3\geq 0$).
...
(jeliko $x_3=0$). Polome tedy $x_1=1$. Dostáváme nové øešení $R_3=(1,2,0,2,0)^T, z(R_3) = 3$. Promìnná
...
\]
Zvìtšíme $x_3$ z $0$ na $2$ ($x_3\leq 2$ plyne ze tøetí rovnice tabulky) a tím obdríme
...
\]
Nyní je jasné, e libovolné zvıšení volné promìnné $x_4$ nebo $x_5$ sníí hodnotu úèelové funkce.
...
Èasová sloitost simplexové metody je $O(2^n)$.
Jeden z nejhorších pøípadù mùeme vzít $n$--dimenzionální krychli,
která má pøesnì $2^n$ vrcholù. Na této krychli algoritmus
mùe postupnì navštívit všechny její vrcholy. 

Simplexová metoda nachází uplatnìní pøevánì pøi øešení optimalizaèních úloh v inenırství nebo ekonomii.

\subsection{Duální úloha}

Problém lineárního programování tak, jak byl popsán vıše, oznaèujeme jako {\it primární}.
Ke kadému primárnímu problému mùeme zkonstruovat {\it duální úlohu}. Pøipomeòme, e
...
%nìco dobrého ke~svaèinì. V pekárnièce prodávají dva druhy zboí -- bábovky a paøíské dorty.
%Bábovka stojí 80 Kè, paøískı dort 50 Kè. Pekárníèku vlastní jedna hodná paní ze sousedství, a proto
%si mùete vzít i jen kousek bábovky nebo dortíku. Na paøískı dortík jsou potøeba
%3 unce èokolády, 2 unce cukru a 2 unce krému. Na bábovku pak jsou potøeba
%4 unce cukru a 5 uncí krému. Jeliko ale Kaèenka dá na správnou vıivu,\footnote{A daøí se jí to pøevelice.}
%ví, e musí jíst hodnotnì, a proto si stanovila, e chce, aby její svaèinka mìla alespoò 6
%uncí èokolády, 10 uncí cukru a 8 uncí krému. Jeliko má ale malinkatou penìenku, do které se jí
%nevešelo moc penízkù, chce, aby si svaèinku splòující daná kritéria nakoupila co nejlevnìji.
%Pomùete milé Kaèence?
%\medskip
...
%\]
%a pøidejme pro kadou takto novì vytvoøenou promìnnou ještì omezující podmínku $x_{n+i}\geq 0$.
%Kadá promìnná $x_{n+i}$ je tedy jakousi \uv{chybou} (rozdílem) hodnoty pravé strany
%a hodnoty levé strany $i$--té nerovnosti.

\begin{e}{Report}{0}{Zahradník} Simplexová metoda. V podstatì jsem mu tam napsal všechno o ní, ale chtìl vysvìtlit kadı detail jak funguje, vèetnì pomocné úlohy, naštìstí jsem to geometricky nìjak chápal, tak mì to nakonec uznal, bokem se zeptal i na duální úlohu a vìty o dualitì. S tím moc spokojenej nebyl. 
\end{e}
\begin{e}{Report}{0}{Fiala} linearni programovani -- zformulovat ulohu LP, popsal jsem mu tam simplexovou metodu, dualni ulohu a vety o dualite.
\end{e}
\begin{e}{Report}{0}{Majerech} Simplexová metoda a vìty o dualitì.
Kdy jsem si to vylosoval tak se Majerech zdìsil a øekl a se teda snaím.
Algoritmus jsem si pamatoval a dùkazy naštìstí nechtìl. Úspìšnì jsem pøedstíral, e vím co se uvnitø toho simplexu dìje a patrnì mi to uvìøil. 

\end{e}
\begin{e}{Report}{0}{Loebl}Linearni programovani: za 1, jen definice a zneni vet, dokazovat jsem nic nemusel a simplexovy algoritmus jsem nijak popisovat nemusel, ze pry je videt, ze to umim (algoritmus jo, dukazy ne, ale ber jak ber :-) ).

\end{e}
\begin{e}{Report}{0}{Dvorak} Na matiku docela rejpal, chtel vedet jak moc tomu rozumim. U simplexovky se ptal, proc tahle uprava tabulky, atd. A co je tohle v tabulce, atd. No jeste, ze jsem chodil na zaklady optimalizace, jinak bych tyhle veci nevedel. Btw: byl rad, ze jsem vedel, ze existuji i jine metody nez simplex - chacijan a karmarkar. 
\end{e}
