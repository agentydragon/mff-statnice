%&latex
\subsubsection{Uzávìrové vlastnosti tøíd jazykù\footnote{tohle asi nikdy nebude samostatna otazka}}
...
\hline          
    & RJ & BKJ & DBKJ & KJ & RJ & RSJ \\          
\hline                         
  Sjednocení & 1 & 1 & 0 & 1 & 1 & 1  \\
\hline            
  Prùnik & 1 & 0 & 0 & 1 & 1 & 1  \\
\hline            
  Prùnik s RJ & 1 & 1 & 1 & ? & ? & ?  \\
\hline 
  Doplnìk & 1 & 0 & 1 & 1 & 1 & 0  \\
\hline 
  Substituce / homomorfismus & 1 & 1 & 0 & 0 & 1 & ?  \\
\hline 
  Inverzní\ \ \ \ \ \ \ \ \ homomorfismus & 1 & 1 & 1 & ? & ? & ?  \\
\hline
\hline  
  Zrcadení $L^R$ & 1 & 1 & ? & 1 & 1 & 1  \\
\hline  
  Zøetìzení & 1 & 1 & ? & 1 & 1 & 1  \\
\hline             
  Iterace $L^*$ & 1 & 1 & ? & 1 & 1 & 1  \\
...
\end{tabular}
\end{center}
Rozdíl lze vyjádøit pomocí prùniku a doplòku take z nich vyplıvá
\end{obecne}
...
paralelní bìh dvou zásobníkovıch automatù: øídící jednotky umíme spojit (viz koneèné automaty), ètení umíme spojit (jeden automat mùe èekat), bohuel dva zásobníky nelze obecnì spojit do jednoho (dva zásobníky = Turingùv stroj, rekurzivnì spoèetné jazyky)
\end{e}

\begin{e}{Poznámka}{0}{(D)BKJ jsou uzavøené na prùnik s RJ}
zásobníkovı a koneènı automat mùeme spojit
...
pouijeme gramatiky (pro ZA nedeterministickı rozeskok na startu)\\
$L_1=L(G_1) G_1 = (VN1,VT1,S1,P1)$\\
$L_2=L(G_2) G_2 = (VN2,VT2,S2,P2)$\\
mùeme pøedpokládat, e VN1 ? VN2 = Ø (jinak pøejmenuj)
udìláme gramatiku:
G = (VN1 ? VN2 ? {S}, VT1 ? VT2,S, P1 ? P2 ? {S › S1 | S2})
zøejmì L(G) = L(G1) ? L(G2)
...
\begin{e}{Poznámka}{0}{Bezkontextové jazyky jsou uzavøeny na substituci}
...
pro kadı terminál ai z  $V_{T0}$, $\sigma(a_i)$ je bezkontextovı jazyk - gramatika $G_i$ pøedpokládejme, e mnoiny neterminálù jsou navzájem disjunktní
a e ádnı terminál není v jiné gramatice neterminálem

definujme $G = (\cup_{i\geq0}V_{Ni}, \cup_{i\geq1}V_{Ti}, S_0, P)$, kde

$P = \cup_{i\geq1}P_i \cup P'$ a $P' = P_0$, kde kadı terminál $a_i$ je nahrazen $S_i$
...
buffer je koneènı, mùeme ho tedy modelovat ve stavu
...
polome $h(0) = 1$ a $h(x) = x$ pro ostatní symboly
...
snadno ošetøíme „podlokou“ na zásobníku\\
...
\begin{e}{Poznámka}{0}{Doplnìk RSJ nemusí vdy bıt RSJ}
Komplementem èistì RSJ (tedy jazyka, kterı je RSJ, ale není rekurzivní) je vdy jazyk, kterı není rekurzivnì spoèetnı. Vezmìme pro pøíklad jazyk HALT, kterı je èistì RSJ, ale jeho komplement (NotHALT) není RSJ. To vyplıvá z definice NotHALT: HALT obsahuje dvojice TS:slovo takové, e TS pro dané slovo zastaví. NotHALT tedy obsahuje dvojice TS:slovo takové, kdy danı TS nezastaví pro slovo, e se zacyklí. Protoe nejsme schopni identifikovat cyklus, nejsme schopni sestavit TS, kterı by jazyk NotHALT pøijímal. Obecnì: pokud máme jazyk L, kterı je èistì RSJ, pak jeho komplement L' musí obsahovat takové slovo, pro nì se TS M' zacyklí a tudí tento TS M' nemùe pøijímat jazyk L'.
\end{e}
