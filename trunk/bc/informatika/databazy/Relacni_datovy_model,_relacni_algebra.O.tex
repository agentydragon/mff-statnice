...
\begin{e}{Definice}{0}{Relaèní datový model}
\emph{Relaèní schéma} je n-tice $R(A_1: D_1, A_2: D_2, ..., A_n: D_n)$, kde množiny $D_i$ jsou tzv. \emph{(atributové) domény}  (odpovídá dat.typùm v tabulkách) - uvádìjí se nepovinnì, množina $A_i$ je \emph{atribut} (odpovídá sloupci hodnot v tabulce) dále definujme \emph{relaci}\footnote{ano $R$ -
relace je nìco jiného než $R(A)$ - relaèní schéma, Skopal bohužel používá
toto trošku matoucí znaèení} $R\subseteq A_1×A_2
...
\begin{e}{Pøíklad}{0}{Relaèní datový model}
definujeme atributy:
\begin{verbatim}
...
\begin{verbatim}
   R = {(Novák, Hlavní, Hradec), (Starý, Severní, Rájec), (Coufal, Severní, Rájec), 
        (Liška, Sadová, Polná)}
\end{verbatim}
je relace na kartézském souèinu (tzn. jeho podmnožina) $jm\acute{e}no × ulice × m\check{e}sto$
\\\\relaèní schéma:
\begin{verbatim}
...
\begin{e}{Definice}{0}{Relaèní algebra}
...
\\$<R,R(A_i\rightarrow B_i, A_j\rightarrow B_j, ... )>$
\item \textbf{sjednocení} - schémata musí mít stejný poèet atributù a kompatibilní
domény
\\$<R_1, R_1(A)>\cup<R_2, R_2(A)>= <R_1\cup R_2 , R_x(A)>$
\item \textbf{rozdíl množin} - schémata musí mít stejný poèet atributù a kompatibilní
domény 
...
\\$<R_1, R_1(A)>\times<R_2, R_2(B)>= <R_1\times R_2 , R_x(\{R_1\}\times A\cup
 \{R_2\} \times B)>$
 \\
 –zde je vhodné si všimnout pravé strany - tím že Skopal ve schématu provedl
 kartézský souèin množiny atributù s jenoprvkovou množinou "$R_1$" vlastnì pøejmenoval atributy na "$R_1A_1,R_2A_2,...$"
 aby nám ve výsledném schématu nevznikly atributy s totožným jménem
\begin{center}
\includegraphics[width=5cm]{informatika/databazy/obrazky/kart_soucin}
\\pøíklad kart.souèinu
\end{center}
...
\item \textbf{projekce} - $u[C]$ je prvek relace zbavený hodnot atributù $A\setminus C$, pøípadné duplicitní prvky jsou odstranìny
\\$<R[C], R(A)>= <\{u[C] \in R\}, R(C)>$ kde $C\subseteq A$

\begin{center}
\includegraphics[width=5cm]{informatika/databazy/obrazky/projekce}
\\pøíklad projekce pro $R[A,C]$
\end{center}

\end{pitemize}
Operace vzniklé skládáním základních operací:
\begin{pitemize}
\item \textbf{prùnik} - schémata musí mít stejný poèet atributù a kompatibilní
domény\\ 
$<R_1, R_1 (A)>\cap<R_2, R_2(A)>= <R_1\cap R_2 , R_x(A)>$
\\\\
–dá se vyjádøit také jako:
$R_1 \cap R_2 = (R_1 \cup R_2 \setminus (R_1 \setminus R_2))\setminus (R_2 \setminus R_1)$

\item \textbf{pøirozené spojení} (natural join) - spojení prvkù relace pøes stejné hodnoty všech atributù sdílených mezi A a B, 

–pokud $A\cap B = \oslash$, pøirozené spojení je vlastnì kartézský souèin (spojuje se pøes prázdnou množinu, tj. libovolnì –„všechno se vším“)

–pokud $A = B$, pøirozené spojení je prùnik (spojí se pouze stejné øádky)

$<R, R(A)> * <S, S(B)>= <\{u | u[A] \in R ~\&~ u[B] \in S\}, R_x(A\cup B)>$

–lze vyjádøit pomocí kartézského souèinu, selekce a projekce -- snad takto (neovìøeno):\\
$R * S = \left(\left(R \times S\right)\left(\varphi\right)\right)[A \cup B]$, kde $\varphi$ je podmínka na rovnost odpovídajících sloupcù $\forall x \in R(A) \cap S(B): Rx = Sx$ (pøejmenované sloupce z kartézského souèinu). Není to úplnì formálnì korektní, protože by to chtìlo pak dìlat závìreènou projekci pøes ty pøejmenované sloupce, ale myšlenka je jasná.

\begin{center}
\includegraphics[width=5cm]{informatika/databazy/obrazky/natural_join}
\\ 
pøíklad pøirozeného spojení
\end{center}

\item \textbf{dìlení} - definujeme si $\oplus$ jako zøetìzení øádkù\\
$<R, R(A)> \div <S, S(B \subset A)>= <\{t | \forall s \in S: (t \oplus s)\in R\}, R_x(A - B)>$
\\
–lze vyjádøit pomocí projekce, rozdílu a kartézského souèinu:
\\
$R \div S = R[A\setminus B] \setminus ((R[A\setminus B] \times S)\setminus
R)[A\setminus B]$

\begin{center}
\includegraphics[width=5cm]{informatika/databazy/obrazky/deleni}
\\ 
pøíklad dìlení
\end{center}
\end{pitemize}
...
\begin{e}{Poznámka}{0}{Relaèní úplnost}
...
\begin{e}{Definice}{0}{Relaèní kalkulus}
...
\begin{e}{Pøíklad}{0}{srovnání RA, DRK, NRK}
...
\\RA:
\\
$ FILM \div HEREC[JMENO\_HERCE] $
\\\\
DRK:\\
$\{(f) | FILM(f)\wedge \forall h (HEREC(h) \Rightarrow FILM(f, h))\}$
\\\\
NRK:\\
$\{film[JMENO\_FILMU] \;|\; \forall herec(HEREC(herec) \Rightarrow f(FILM(f) \wedge\; f.JMENO\_HERCE = herec.JMENO\_HERCE \;\wedge f.JMENO\_FILMU = film.JMENO\_FILMU))\}$
\end{e}
